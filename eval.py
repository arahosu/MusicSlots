...
from models.Baselines.vae import VanillaVAE
...
def main(argv):
    ...
    if 'kld_weight' in ckpt['hyper_parameters']:
        model = VanillaVAE.load_from_checkpoint(FLAGS.model_dir)
    else:
        model = SlotAttentionAE.load_from_checkpoint(FLAGS.model_dir, num_iter=FLAGS.test_time_iter)

    if FLAGS.log_wandb:
        if FLAGS.name is None:
            name = 'eval_run' + '_{}'.format(model.__class__.__name__)
        else:
            name = FLAGS.name
